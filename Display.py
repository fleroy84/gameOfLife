...
class Display :
    ...
    def __init__(self, win, cellSize, height, width, world):
        ...
        #liste d√©roulante pour choisir la r√®gle du jeu
    ...
    def __load(self):
        ...
        if self.configFile:
            self.__world.raz() #on rÈinitialise la grille
    ...
    def refreshLabel(self, init_generation=False): #on passe en parametre le booleen True ‡ la methode si l on souhaite initialiser le compteur de generatÏon ‡ 0 apres execution de play() 
        if init_generation:
            self.generation=0
